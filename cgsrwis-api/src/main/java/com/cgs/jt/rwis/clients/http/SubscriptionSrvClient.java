...
/**
 * Utility class to access the endpoints provided by the subscription service (one of the dropwizard-based services
...
public class SubscriptionSrvClient {	
	...
	/**
	 * Represents the web target pointing to the root path of the subscription service.
	 */
	 ...
	 *  
	 */
	public SubscriptionSrvClient(URI uri){
		...
		//Create webtarget
		this.target = client.target(uri).path(SubscriptionServiceConf.SUBSCR_ROOT_PATH);
	...
	/**
	 * Sends HTTP request containing the identification of forecast model (in query param), obtains HTTP response, and - if successful - returns
	 * all the "parameter subscriptions" associated with the given forecast model (in other words: all the locations and for 
	 * each location the associated (weather) parameters - that are forecasted by the given forecast model). 
	 ...
	 * @return The locations each with a set of associated (weather) parameters - that are served by the given
	 *  forecast model, or null if HTTP status 204 No Content is returned (NOTE: 204 No Content 
	...
	public HashMap<EarthSurfacePoint, HashSet<ForecastedParameter>> getParameterSubscriptionsByModel(String forecastModelId) throws HttpRequestExecutionException{
		...
		try {
			response = target
					.path(SubscriptionServiceConf.GET_FORECAST_PARAM_SUBSCR_BY_MODEL_PATH)
					.queryParam(SubscriptionServiceConf.QUERYPARAM_FORECAST_MODEL_ID , forecastModelId)					
			...
			if (response.getStatusInfo().equals(Response.Status.OK)){				
				...
				HashMap<EarthSurfacePoint, HashSet<ForecastedParameter>> result = response.readEntity(new GenericType<HashMap<EarthSurfacePoint, HashSet<ForecastedParameter>>>() { });
				return result;
			}
	 ...
	 */
	public void insertParameterForecastSubscription(ParameterForecastSubscription pfs) throws HttpRequestExecutionException {
		...
		try {
			response = target
					.path(SubscriptionServiceConf.INSERT_FORECAST_PARAM_SUBSCRIPTIONS_PATH)
					.request(MediaType.APPLICATION_JSON)				
	...
	/**
	 * Sends HTTP request containing the identification of forecast model (in query param), obtains HTTP response, and - if successful - returns
	 * all the "visualizations subscriptions" associated with the given forecast model (in other words: all the weather parameters 
	 * that are forecasted by the given forecast model and for which the weather visualization (i.e. image showing weather forecast on the entire
	 * weather forecast model grid) needs to be created. 
	 * @param forecastModelId The forecast model for which to find out which visualizations need to be created.
	 * @return The weather forecast parameters (forecasted by the given model) that need to be visualized, or null if HTTP status 204 No Content 
	 * is returned (NOTE: 204 No Content is for example returned by Jetty server if null is returned on the Dropwizard service endpoint - for example
	 * because nothing is found in the database).
	 */	
	public HashSet<ForecastedParameter> getVisualizationSubscriptionsByModel(String forecastModelId) throws HttpRequestExecutionException{
		//TODO: seems like Response does not support try-with-resources does it??? 
		//NOTE: Closing the response is not strictly necessary (if not using InputStream), but it does not hurt anyway - see:
		//https://stackoverflow.com/questions/33083961/closing-jax-rs-client-response
		Response response = null;
		try {
			response = target
					.path(SubscriptionServiceConf.GET_VISUALIZATION_SUBSCR_BY_MODEL_PATH)
					.queryParam(SubscriptionServiceConf.QUERYPARAM_FORECAST_MODEL_ID , forecastModelId)					
					.request(MediaType.APPLICATION_JSON)
					.get();
			if (response.getStatusInfo().equals(Response.Status.OK)){				
				//NOTE: when reading object entity in JAX-RS client you need to provide the GenericType instance (a hint 
				//to JAX-RS how to deserialize)
				HashSet<ForecastedParameter> result = response.readEntity(new GenericType<HashSet<ForecastedParameter>>() { });
				return result;
			}
			else if (response.getStatusInfo().equals(Response.Status.NO_CONTENT)){
				//if Dropwizard service resource endpoint returned null (for example if nothing was found in the database) - then 
	...
	public MetroLocationDescription getMetroLocationDescription(GeographicLocation geoLocation, String modelId) throws HttpRequestExecutionException{
		...
		try {
			response = target
					.path(SubscriptionServiceConf.GET_METRO_LOC_DESCR_PATH)
					.queryParam(SubscriptionServiceConf.QUERYPARAM_LOC_LAT , geoLocation.getLatitude())					
					.queryParam(SubscriptionServiceConf.QUERYPARAM_LOC_LON , geoLocation.getLongitude())
					.queryParam(SubscriptionServiceConf.QUERYPARAM_FORECAST_MODEL_ID, modelId)
					.request(MediaType.APPLICATION_JSON)
					.get();
			if (response.getStatusInfo().equals(Response.Status.OK)){				
	 ...
	 */	
	public void insertMetroLocationDescription(MetroLocationDescription locationDescription) throws HttpRequestExecutionException{
		...
		try {
			response = target
					.path(SubscriptionServiceConf.INSERT_METRO_LOC_DESCR_PATH)
					.request(MediaType.APPLICATION_JSON)				
	...
	/**
	 * Sends HTTP request containing the identification of the region (in query param), obtains HTTP response, and - if successful - returns
	 * all the "reference points descriptions" associated with the given region. 
	 * @param region The region for which to find the associated reference points.
	 * @return The reference points descriptions, or null if HTTP status 204 No Content is returned (NOTE: 204 No Content 
	 * is for example returned by Jetty server if null is returned on the Dropwizard service endpoint - for example because nothing is 
	 * found in the database).
	 */
	public HashSet<ReferencePointDescription> getReferencePointsDescriptionsByRegion(String region) throws HttpRequestExecutionException{
		...
		try {
			response = target
					.path(SubscriptionServiceConf.GET_REF_POINTS_DESCR_BY_REGION_PATH)
					.queryParam(SubscriptionServiceConf.QUERYPARAM_REGION, region)					
					.request(MediaType.APPLICATION_JSON)
					.get();
			if (response.getStatusInfo().equals(Response.Status.OK)){				
				//NOTE: when reading Set entity in JAX-RS client you need to provide the GenericType instance (a hint 
				//to JAX-RS how to deserialize)
				HashSet<ReferencePointDescription> result = response.readEntity(new GenericType<HashSet<ReferencePointDescription>>() { });
				return result;
			}
			else if (response.getStatusInfo().equals(Response.Status.NO_CONTENT)){
				//if Dropwizard service resource endpoint returned null (for example if nothing was found in the database) - then 
	...
	/**
	 * Sends HTTP request containing the ReferencePointDescription, obtains HTTP response,
	 * checks the response, and if not 200 OK throws exception. 
	 * 
	 * @param rpDes The reference point description object to sent to the service. 
	 */	
	public void insertReferencePointDescription(ReferencePointDescription rpDes) throws HttpRequestExecutionException{
		...
		try {
			response = target
					.path(SubscriptionServiceConf.INSERT_REF_POINT_DESCR_PATH)
					.request(MediaType.APPLICATION_JSON)				
					.post(Entity.json(rpDes));
